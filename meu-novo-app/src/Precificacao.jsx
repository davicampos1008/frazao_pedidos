...
export default function Precificacao() {
  ...
  const finalizarTeste = async () => {
    ...
    if (!error) {
      setTesteAtivo(false);
      setConfigGlobal({ is_feriado: false, nao_funciona: false, data_teste: '' });
      alert("✅ Teste Finalizado!");
    }
    setCarregando(false);
  };

  const salvarConfigBasica = async (novaConfig) => {
    setConfigGlobal(novaConfig);
    await supabase.from('configuracoes').update({
      is_feriado: novaConfig.is_feriado,
      nao_funciona: novaConfig.nao_funciona
    }).eq('id', 1);
  };
  ...
  const revisarItensOcultos = async () => {
    ...
    try {
      ...
      if (loteUpdates.length > 0) {
        ...
        alert(`✅ REVISÃO CONCLUÍDA!\nResgatamos ${loteUpdates.length} produtos novos.`);
        setProdutos(novaLista);
      } else {
        alert("✔️ Nenhum produto novo aguardando revisão.");
      }
  ...
  const finalizarCotacao = async () => {
    if (qtdPendentes > 0) {
      if (!window.confirm(`⚠️ ATENÇÃO: Você ainda tem ${qtdPendentes} itens PENDENTES.`)) return;
    }
    setCarregando(true);
    const { error } = await supabase.from('configuracoes').update({ precos_liberados: true }).eq('id', 1);
    setCarregando(false);
    if (!error) alert("🚀 LOJA ABERTA!");
  };
  ...
  const salvarModal = async () => {
    ...
    if (statusCalc === 'ativo' || statusCalc === 'mantido') payload.preco_anterior = payload.preco;
    setCarregando(true);
  ...
  return (
    <div style={{ width: '100%', maxWidth: '900px', margin: '0 auto', fontFamily: 'sans-serif', padding: '10px' }}>
      ...
      {/* ⚙️ BOTÃO ENGRENAGEM E PAINEL CONFIG */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '10px' }}>
        <button onClick={() => setPanelConfigAberto(!panelConfigAberto)} style={{ background: '#333', color: '#fff', border: 'none', width: '45px', height: '45px', borderRadius: '12px', fontSize: '20px', cursor: 'pointer' }}>⚙️</button>
      </div>

      {panelConfigAberto && (
        <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '20px', marginBottom: '20px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', border: '1px solid #e2e8f0' }}>
          <h3 style={{ margin: '0 0 15px 0', fontSize: '16px', fontWeight: '900' }}>Configurações do Sistema</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
            <button onClick={() => salvarConfigBasica({...configGlobal, nao_funciona: !configGlobal.nao_funciona})} style={{ padding: '12px', borderRadius: '12px', border: 'none', fontWeight: 'bold', cursor: 'pointer', background: configGlobal.nao_funciona ? '#ef4444' : '#f1f5f9', color: configGlobal.nao_funciona ? '#fff' : '#444' }}>
              {configGlobal.nao_funciona ? '🚫 LOJA FECHADA' : '✅ LOJA ABERTA'}
            </button>
            <button onClick={() => salvarConfigBasica({...configGlobal, is_feriado: !configGlobal.is_feriado})} style={{ padding: '12px', borderRadius: '12px', border: 'none', fontWeight: 'bold', cursor: 'pointer', background: configGlobal.is_feriado ? '#eab308' : '#f1f5f9', color: configGlobal.is_feriado ? '#fff' : '#444' }}>
              {configGlobal.is_feriado ? '🚩 É FERIADO' : '🏳️ DIA NORMAL'}
            </button>
          </div>
          <div style={{ padding: '15px', background: '#f8fafc', borderRadius: '15px' }}>
            <label style={{ fontSize: '12px', fontWeight: 'bold', color: '#64748b', display: 'block', marginBottom: '8px' }}>SIMULAR DATA (TESTE):</label>
            <div style={{ display: 'flex', gap: '8px' }}>
              <input type="date" value={configGlobal.data_teste} onChange={e => setConfigGlobal({...configGlobal, data_teste: e.target.value})} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1' }} />
              <button onClick={aplicarTeste} style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '10px 15px', borderRadius: '8px', fontWeight: 'bold' }}>APLICAR</button>
            </div>
            {testeAtivo && (
               <button onClick={finalizarTeste} style={{ width: '100%', marginTop: '10px', background: '#ef4444', color: '#fff', border: 'none', padding: '10px', borderRadius: '8px', fontWeight: 'bold' }}>FINALIZAR TESTE</button>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{ backgroundColor: '#111', padding: '25px', borderRadius: '24px', color: 'white', marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ margin: '5px 0 0 0', color: '#94a3b8', fontSize: '13px' }}>Faltam <strong style={{color: '#ef4444'}}>{qtdPendentes}</strong> itens.</p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <button onClick={revisarItensOcultos} style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '10px 15px', borderRadius: '10px', fontWeight: '900', fontSize: '11px' }}>🔄 REVISAR</button>
            <button onClick={zerarCotacao} style={{ background: '#333', color: '#ef4444', border: 'none', padding: '10px 15px', borderRadius: '10px', fontWeight: '900', fontSize: '11px' }}>🗑️ ZERAR</button>
          </div>
        </div>
      ...
      {/* ABAS */}
      <div style={{ display: 'flex', gap: '5px', marginBottom: '15px', overflowX: 'auto', scrollbarWidth: 'none' }}>
        {CONFIG_ABAS.map(aba => (
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '12px', display: 'flex', gap: '10px', marginBottom: '15px' }}>
        <span>🔍</span><input placeholder="Buscar item..." value={busca} onChange={e => setBusca(e.target.value)} style={{ border: 'none', width: '100%', outline: 'none' }} />
      </div>

      {/* LISTA */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {listas[abaAtiva].filter(p => p.nome.toLowerCase().includes(busca.toLowerCase())).map(p => (
           <LinhaProduto key={p.id} produto={p} corBorda={CONFIG_ABAS.find(a => a.id === p.status_cotacao)?.cor || '#3b82f6'} abrirModal={abrirEdicaoCompleta} aoSalvar={handleAtualizarLista} />
        ))}
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', justifyContent: 'center', alignItems: 'flex-end' }}>
          <div style={{ backgroundColor: '#fff', width: '100%', maxWidth: '600px', borderRadius: '30px 30px 0 0', padding: '30px 25px', display: 'flex', flexDirection: 'column', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h3 style={{ margin: 0 }}>✏️ Edição Completa</h3>
              <button onClick={() => setModalAberto(false)} style={{ border: 'none', background: 'none', fontSize: '20px' }}>✕</button>
            </div>
            <strong style={{ marginBottom: '20px', display: 'block' }}>{prodModal.nome}</strong>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '25px' }}>
               <button onClick={() => setFormModal({...formModal, status: 'mantido'})} style={{ padding: '10px', borderRadius: '10px', border: formModal.status === 'mantido' ? '2px solid #eab308' : '1px solid #ddd' }}>🔒 MANTER</button>
               <button onClick={() => setFormModal({...formModal, status: 'sem_preco'})} style={{ padding: '10px', borderRadius: '10px', border: formModal.status === 'sem_preco' ? '2px solid #f97316' : '1px solid #ddd' }}>⏸️ S/ PREÇO</button>
               <button onClick={() => setFormModal({...formModal, status: 'falta'})} style={{ padding: '10px', borderRadius: '10px', border: formModal.status === 'falta' ? '2px solid #ef4444' : '1px solid #ddd' }}>🚫 FALTA</button>
            </div>
            <input type="text" value={formModal.preco} onChange={e => setFormModal({...formModal, preco: e.target.value})} placeholder="Preço" style={{ width: '100%', padding: '15px', borderRadius: '10px', border: '1px solid #ddd', marginBottom: '15px' }} />
            <button onClick={salvarModal} style={{ width: '100%', padding: '20px', backgroundColor: '#111', color: 'white', border: 'none', borderRadius: '16px', fontWeight: '900' }}>💾 SALVAR</button>
          </div>
        </div>
      )}
    </div>
