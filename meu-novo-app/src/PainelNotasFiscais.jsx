...
export default function PainelNotasFiscais({ isEscuro }) {
  ...
  async function carregarDados() {
    ...
    try {
      ...
      // =======================================================================
      // 1. AGRUPAR POR FORNECEDOR (Mesma Lógica Perfeita do Fechamento)
      // =======================================================================
      const mapaForn = {};
      const initNFs = {};
      ...
      pedidos.forEach(p => {
        ...
        if (!mapaForn[fNome]) {
          ...
          mapaForn[fNome] = { 
              ...
              itensRaw: {}, // 💡 Usando o Raw para somar os iguais e separar os diferentes
              itens: [] 
          };
          if (p.nota_fiscal) initNFs[fNome] = p.nota_fiscal;
        }

        // 💡 Cálculo focado no custo_unit real
        ...
        // Chave única: Nome + Preço Custo + Boleto
        ...
        // Recupera NF preenchida em qualquer item do mesmo fornecedor
      ...
      // Converte o Raw para o array final que a tela entende
      ...
      // =======================================================================
      // 2. AGRUPAR POR LOJA (Mantém agrupamento original da tela fiscal)
      // =======================================================================
      const mapaLoja = {};
      pedidos.forEach(p => {
        ...
        // Lojas priorizam a visualização do custo de venda (média), se não tiver, o de custo.
  ...
  const salvarNotaFiscal = async (fornecedorNome) => {
    ...
    try {
      const { error } = await supabase.from('pedidos')
        ...
        .eq('data_pedido', hoje)
        .ilike('fornecedor_compra', `%${fornecedorNome}%`); // Cobre boletos e normais
                      ...
                      <div style={{ marginTop: '20px', paddingTop: '15px', borderTop: `1px solid ${corStatus}50` }}>
                        ...
                        {f.statusCadastro === 'LINKADO' && (
                          <div style={{ background: isEscuro ? '#14532d' : '#dcfce7', padding: '15px', borderRadius: '12px', color: isEscuro ? '#86efac' : '#166534', border: `1px solid ${configDesign.cores.sucesso}`, marginBottom: '15px', fontSize: '12px' }}>
                             ...
                             <br/>
                             <strong style={{color: configDesign.cores.textoForte}}>Titular PIX:</strong> {f.dadosCadastro.nome_titular_pix || 'Não informado'}
                             <br/>
                        ...
                        {f.statusCadastro === 'LINKADO' && f.tipoPessoa === 'PJ' ? (
                            <div style={{ background: configDesign.cores.fundoGeral, padding: '15px', borderRadius: '12px', border: `1px solid ${configDesign.cores.borda}` }}>
