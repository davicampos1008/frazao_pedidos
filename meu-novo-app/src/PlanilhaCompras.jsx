...
export default function PlanilhaCompras() {
  ...
  // 💡 FUNÇÃO PARA CANCELAR ITEM ÚNICO
  ...
  // 💡 FUNÇÃO PARA ATUALIZAR NOME DO FORNECEDOR EM LOTE
  ...
  const carregarDados = useCallback(async (silencioso = false) => {
    ...
    try {
      ...
      (pedData || []).forEach(p => {
        ...
        if (idLoja !== null && idLoja >= 0) { 
          ...
          if (!mapaGeralItens[nomeProdutoUpper]) {
            mapaGeralItens[nomeProdutoUpper] = { nome: nomeProdutoUpper, unidade: String(p.unidade_medida || "UN"), total_solicitado: 0, total_comprado: 0, isFaltaTotal: false, temBoleto: false, fornecedores_comprados: {} };
          }
          ...
          if (p.status_compra === 'atendido' || p.status_compra === 'boleto') {
             ...
             if (fName && fName !== 'REFAZER') {
                if (!mapaForn[fName]) {
                    const fInfo = (fornData || []).find(f => (f.nome_fantasia || '').toUpperCase() === fName);
                    mapaForn[fName] = { nome: fName, totalGeral: 0, lojas: {}, temCadastro: !!fInfo, alertas: [] };
                }
      ...
      setDemandas(Object.values(mapaPendentes).map(item => {
        ...
        return { ...item, preco_sugerido: prodRef ? prodRef.preco : 'R$ 0,00', fornecedor_sugerido: prodRef && prodRef.fornecedor_nome ? prodRef.fornecedor_nome : 'Não cadastrado' };
      }).sort((a, b) => a.nome.localeCompare(b.nome)));
      setPedidosFeitos(Object.values(mapaFeitos).sort((a, b) => a.nome.localeCompare(b.nome)));
      
      const fornProcessados = Object.values(mapaForn).sort((a, b) => a.nome.localeCompare(b.nome));
  ...
  const resetarPedidosDoDia = async () => {
    if (!window.confirm(`🚨 ATENÇÃO: ZERAR TUDO?`)) return;
    setCarregando(true);
  ...
  const aceitarBonificacaoCliente = () => {
     ...
     setLojasEnvolvidas(lojasEnvolvidas.map(l => ({ ...l, qtd_bonificada: l.qtd_bonificada_cliente || 0 })));
  };
  ...
  const finalizarPedidoCompleto = () => {
    ...
    const promessas = lojasEnvolvidas.map(loja => supabase.from('pedidos').update({
      fornecedor_compra: dadosCompra.fornecedor.toUpperCase(), custo_unit: precoFinal, qtd_atendida: loja.qtd_pedida, qtd_bonificada: loja.qtd_bonificada, status_compra: loja.isBoleto ? 'boleto' : 'atendido'
  ...
  const finalizarPedidoFracionado = () => {
    ...
    setItemModal(null);
    setCarregando(true);
    const promessas = lojasEnvolvidas.map(loja => {
       if (Number(loja.qtd_receber) > 0) {
          return supabase.from('pedidos').update({
            fornecedor_compra: dadosCompra.fornecedor.toUpperCase(), custo_unit: precoFinal, qtd_atendida: Number(loja.qtd_receber), qtd_bonificada: loja.qtd_bonificada, status_compra: loja.isBoleto ? 'boleto' : 'atendido'
          }).eq('id', loja.id_pedido);
       }
       return null;
    }).filter(p => p !== null);
    ...
    grupo.itens.forEach(nomeItem => {
      ...
      demandaItem.lojas.forEach(loja => {
        promessas.push(supabase.from('pedidos').update({
          fornecedor_compra: grupo.fornecedor, custo_unit: `R$ ${precoDigitado}`, qtd_atendida: loja.qtd_pedida, qtd_bonificada: loja.qtd_bonificada_cliente || 0, status_compra: tipoFechamento === 'boleto' ? 'boleto' : 'atendido'
        }).eq('id', loja.id_pedido));
      });
    });
  ...
  const removerGrupoFornecedor = (idGrupo) => {
    if(window.confirm("Deseja desfazer este grupo?")) {
  ...
  return (
    <div style={{ width: '100%', maxWidth: '900px', margin: '0 auto', fontFamily: 'sans-serif', paddingBottom: '120px', padding: '10px' }}>
      ...
      {/* DATALIST GLOBAL - USANDO NOMES DO BANCO */}
      <datalist id="lista-fornecedores">
        {fornecedoresBd.map((f, i) => <option key={i} value={f.nome} />)}
      </datalist>
      ...
      <div style={{ backgroundColor: '#111', padding: '25px', borderRadius: '24px', color: 'white', marginBottom: '20px' }}>
        ...
        <input type="date" value={dataFiltro} onChange={(e) => setDataFiltro(e.target.value)} style={{ background: '#333', color: '#fff', border: 'none', marginTop: '10px', padding: '5px' }} />
      ...
      <div style={{ display: 'flex', gap: '5px', marginBottom: '15px', overflowX: 'auto' }}>
        ...
        <button onClick={() => setAbaAtiva('feitos')} style={{ padding: '15px', border: 'none', borderRadius: '12px', background: abaAtiva === 'feitos' ? '#3b82f6' : '#fff' }}>✅ FEITOS</button>
        ...
        <div>
          ...
          {itensSelecionados.length > 0 && (
            <div style={{ position: 'fixed', bottom: 0, width: '100%', background: '#fff', padding: '20px', left: 0, boxShadow: '0 -5px 10px rgba(0,0,0,0.1)' }}>
               {/* 💡 INPUT COM DATALIST NO AGRUPAMENTO */}
               <input list="lista-fornecedores" placeholder="Nome Fornecedor..." value={nomeFornecedorLote} onChange={e => setNomeFornecedorLote(e.target.value)} style={{ padding: '15px', width: '70%', borderRadius: '12px', border: '1px solid #ddd' }} />
               <button onClick={criarGrupoFornecedor} style={{ background: '#8b5cf6', color: '#fff', padding: '15px', borderRadius: '12px', border: 'none', marginLeft: '10px' }}>AGRUPAR</button>
            </div>
          )}
        </div>
      )}
      ...
      {abaAtiva === 'fornecedores' && (
        <div>
          {fornecedoresBd.map(f => (
            <div key={f.nome} style={{ background: '#fff', borderRadius: '20px', padding: '20px', marginBottom: '10px', borderTop: f.temCadastro ? '6px solid #111' : '6px solid #ef4444' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  {editandoNomeForn === f.nome ? (
                    <div style={{ display: 'flex', gap: '5px' }}>
                      <input list="lista-fornecedores" value={novoNomeForn} onChange={e => setNovoNomeForn(e.target.value)} style={{ padding: '5px' }} />
                      <button onClick={salvarNovoNomeFornecedor} style={{ background: '#22c55e', color: '#fff', border: 'none', borderRadius: '4px', padding: '5px 10px' }}>OK</button>
                      <button onClick={() => setEditandoNomeForn(null)} style={{ background: '#64748b', color: '#fff', border: 'none', borderRadius: '4px', padding: '5px 10px' }}>X</button>
                    </div>
                  ) : (
              ...
              <div onClick={() => setFornExpandido(fornExpandido === f.nome ? null : f.nome)} style={{ cursor: 'pointer', marginTop: '10px', color: '#666' }}>Lojas ▼</div>
              {fornExpandido === f.nome && (
                <div style={{ marginTop: '15px' }}>
                  {Object.values(f.lojas).map(loja => (
                    <div key={loja.nome} style={{ background: '#f8fafc', padding: '10px', borderRadius: '12px', marginBottom: '5px' }}>
                      <strong>{loja.nome}:</strong>
                      {loja.itens.map(it => (
                        <div key={it.id_pedido} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginTop: '5px' }}>
                          <span>{it.qtd}x {it.nome} - {it.valor_unit}</span>
                          {/* 💡 BOTÃO DE CANCELAR ITEM ÚNICO */}
      ...
      {itemModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ backgroundColor: '#fff', width: '90%', maxWidth: '500px', padding: '30px', borderRadius: '30px' }}>
            <h3>Lançar: {itemModal.nome}</h3>
            {/* 💡 INPUT COM DATALIST NO MODAL */}
            <input list="lista-fornecedores" placeholder="Fornecedor..." value={dadosCompra.fornecedor} onChange={(e) => setDadosCompra({...dadosCompra, fornecedor: e.target.value})} style={{ width: '100%', padding: '15px', marginBottom: '10px', borderRadius: '12px', border: '1px solid #ddd' }} />
            <input type="text" placeholder="Preço (R$)" value={dadosCompra.valor_unit} onChange={(e) => setDadosCompra({...dadosCompra, valor_unit: e.target.value})} style={{ width: '100%', padding: '15px', marginBottom: '10px', borderRadius: '12px', border: '1px solid #ddd' }} />
            <button onClick={abaModal === 'completo' ? finalizarPedidoCompleto : finalizarPedidoFracionado} style={{ width: '100%', padding: '15px', background: '#111', color: '#fff', borderRadius: '12px', border: 'none', fontWeight: 'bold' }}>SALVAR</button>
            <button onClick={() => setItemModal(null)} style={{ width: '100%', marginTop: '10px', border: 'none', background: 'none' }}>Sair</button>
          </div>
        </div>
      )}
